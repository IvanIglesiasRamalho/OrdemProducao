...
class SistemaOrdemProducao:
    ...
    def listar_materiais_por_estrutura(self, produto_pai_id: int, quantidade_op: float) -> List[Dict[str, Any]]:
        ...
        self.cursor.execute(q, (produto_pai_id,))
        rows = self.cursor.fetchall()

        itens: List[Dict[str, Any]] = []
        for (_, componente_id, qtd_por, nome, sku) in rows:
            try:
                qtd_por_f = float(qtd_por or 0.0)
            except Exception:
                qtd_por_f = 0.0

            qtd_total = qtd_por_f * float(quantidade_op)
...
class OrdemProducaoApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # NOVO: materiais/estrutura
        ttk.Button(qtd_frame, text="Materiais (Estrutura) / Excel (F7)",
                   command=self.mostrar_materiais_estrutura).grid(row=0, column=2, padx=(0, 5))
        ...
        self.bind("<F7>", lambda e: self.mostrar_materiais_estrutura())
